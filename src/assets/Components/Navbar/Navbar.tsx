...
interface NavbarProps {
  setIsShowModaleCounseling: React.Dispatch<React.SetStateAction<boolean>>;
}
...
export default function Navbar({
  ...
  return (
    <>
      <div
        className={`bg-white  flex mt-2.5 items-center justify-between p-4 ${
          isFixed ? "fixed top-3" : ""
        }  right-0 left-0 rounded-sm`}
      >
        <div className="cursor-pointer ">
          <svg
            onClick={() => setOpen(!open)}
            className="w-8 h-8 sm:hidden flex"
          >
            <use href="#bars-3"></use>
          </svg>
        </div>
        ...
        <div  onClick={() => setIsShowModaleCounseling(true)} className="cursor-pointer bg-primary sm:rounded-bl-full  sm:rounded-tr-full pt-2 pb-2 text-white pr-4 pl-4 rounded-sm  sm:rounded-0">
          <span  className="text-xs sm:text-sm w-full z-20">نیاز به مشاوره دارد؟</span>
        </div>
